...
#
#   VERSION     Major OS version number (e.g., '9', '8', '7')
#   YUM_SERVER  (Optional) empty, 'local', or a URI of a yum server
#   LINUX_DIST  (Optional) Forces Linux  distro (e.g., 'CentOS', 'RedHat')
#
# Supported OSes: LINUX_DIST: [CentOS, RedHat],  VERSION: [7, 8, 9]
#   NOTE: RedHat 9 only -- CentOS 9 media is not produced by this branch.
#
...
if [ "$distro" == RedHat ]; then
  ...
  case $version in
    '9' )
      # EL9 SIMP ISO carries only the base OS repos plus SimpRepos/{SIMP,puppet}.
      # There is no epel, postgresql or codeready-builder repo on this media --
      # every package the kickstart needs resolves from BaseOS/AppStream, except
      # openvox-server/openvox-agent (in the 'puppet' repo) and simp/simp-adapter
      # (in the 'SIMP' repo).
