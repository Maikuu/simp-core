...
An earlier revision of this section claimed the `--pbkdf=pbkdf2` change in
`ks/diskdetect.sh` makes `fips-mode-setup --enable` succeed on its own terms, so
the `||` fallback never fires. **That was wrong**, and an install from the ISO
disproved it.

Anaconda accepts the option -- it is recorded verbatim in the installed
`anaconda-ks.cfg`, and blivet builds a `LUKS2PBKDFArgs` object from it -- and the
keyslot is still formatted `argon2id`:

```
anaconda-ks.cfg:  part pv.141 ... --encrypted --luks-version=luks2 --pbkdf=pbkdf2
luksDump:         Keyslots:  0: luks2 ... PBKDF: argon2id
anaconda logs:    The following encrypted devices use Argon2 PBKDF: /dev/sda5(luks-...)
```

Beware the trap that nearly hid this: `luksDump` prints `pbkdf2` under
**Digests:**, which is a *different field* and is always pbkdf2 on LUKS2. Only
the **Keyslots:** section answers the question.

The resulting system boots and unlocks normally, but reports:
...
`fips_enabled` is 1 and the crypto policy is FIPS -- but that policy came from
the `||` fallback in `pupmod-simp-fips`, not from a `fips-mode-setup` that
agreed to grant it. A compliance defect on a box intended for STIG, not a boot
problem.

### The actual fix

`ks/dvd/auto.cfg` already runs a `luksChangeKey` in `%post --nochroot`, which is
before the chrooted `%post` where FIPS is enabled. Convert the keyslot there:

```sh
cryptsetup luksChangeKey --pbkdf pbkdf2 --key-slot 0 \
  --key-file /boot/disk_creds $crypt_partition /boot/disk_creds
```

followed by an assertion that fails the install rather than shipping a box that
will report an inconsistent FIPS state:

```sh
slot_pbkdf=`cryptsetup luksDump $crypt_partition \
  | sed -n '/^Keyslots:/,/^Tokens:/p' | awk '/PBKDF:/ {print $2; exit}'`
[ "$slot_pbkdf" = "pbkdf2" ] || exit 1
```

The parse was checked against a known-bad node: it returns `argon2id` and does
not pick up the `pbkdf2` from the Digests section.

`--pbkdf=pbkdf2` stays in `diskdetect.sh` -- it is correct to ask for, costs
nothing, and will start working if the anaconda/blivet bug is fixed.
